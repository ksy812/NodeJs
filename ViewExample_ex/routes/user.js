	...
	if (database.db) {
		authUser(database, paramId, paramPassword, function (err, docs) {
			...
			if (docs) {
				...
				//view - login 수정1 후 시작 - login_success.ejs
				...
				//view - login 수정2 후 시작 - login_fail.ejs
				var context = { userid: paramId, userpassword: paramPassword };
	...
	if (database.db) {
		addUser(database, paramId, paramPassword, paramName, function (err, addedUser) {
			...
			if (addedUser) {
				...
				//view - login 수정5 후 시작 - adduser.ejs
				// 뷰 템플레이트를 이용하여 렌더링한 후 전송-view-adduser 시작
...
var listuser = function (req, res) {
	...
	if (database.db) {
		...
		database.UserModel.findAll(function (err, results) {
			...
			if (results.length > 0) { //view-listuser 수정4와 함께 변경 ( if (results) )
				console.dir(results);
				//view - login 수정3 후 시작 - listuser.ejs
				// 뷰 템플레이트를 이용하여 렌더링한 후 전송
