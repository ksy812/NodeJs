...
function createSchema(app, config) {
    ...
    for (var i = 0; i < schemaLen; i++) {
        ...
        var curSchema = require(curItem.file).createSchema(mongoose); //user_schema.js로 전달
        console.log('%s 모듈을 불러들인 후 스키마 정의함.', curItem.file);

        // User 모델 정의
        var curModel = mongoose.model(curItem.collection, ucrSchema);
