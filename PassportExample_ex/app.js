...
router.route('/signup').get(function(req, res) {
	console.log('/signup 패 스 요청됨.');
	res.render('signup.ejs', {message: req.flash('signupMessage')});
});

// 회원가입 - POST로 요청받으면 패스포트를 이용해 회원가입 유도함
// 인증 확인 후, 성공 시 /profile 리다이렉트, 실패 시 /sighup으로 리다이렉트 함
// 인증 실패 시 검증 콜백에서 설정한 플래시 메시지가 응답 페이지에 전달되도록 함
...
router.route('/profile').get(function(req, res) {
	...
	// 인증된 경우, req.user 객체에 사용자 정보 있으며, 인증안된 경우 req.user는 false 값임
...
passport.use('local-login', new LocalStrategy({ //a
	...
	database.UserModel.findOne({ 'email':email }, function(err, user) { //b
		...
		if(!user) {
			...
			// 검증 콜백에서 두 번쨰 파라미터 값을 false로 하여 인증 실패한 것으로 처리
		}
...
passport.use('local-signup', new LocalStrategy({ //a
	...
	process.nextTick(function() { //b
		var database = app.get('database');
		database.UserModel.findOne({ 'email': email }, function(err, user) { //ㅊ
			// 에러 발생 시
			if(err) {
				return done(err);
			}
			...
			if(user) {
				...
				user.save(function(err) { //e
					...
					return done(null, user);
					// 검증 콜백에서 두 번쨰 파라미터의 값을 user 객체로 넣어 인증 성공한 것으로 처리
				}); //e
			} //d
		});  //c
	}); //b
