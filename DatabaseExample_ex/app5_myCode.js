...
function createUserSchema(){
	...
	// 스키마 정의
	//password를 hashed_apssword로 변경, 각 컬럼에 default 속성 모두 추가, salt 속성 추가
	UserSchema = mongoose.Schema({
		id: {type: String, required: true, unique: true, 'default':''},
		//password: {type: String, required: true},
		hashed_password: {type: String, required: true, 'defualt':''},
	...
	UserSchema.method('authenticate', function(plainText, inSalt, hashed_password){
		if(inSalt){
			console.log('authenticate 호출됨: %s -> %s : %s',
				plainText, this.encryptPassword(plainText, inSalt), hashed_password);
			return this.encryptPassword(plainText, inSalt) === hashed_password;
		} else {
			console.log('authenticate 호출됨: %s -> %s : %s',
				plainText, this.encryptPassword(plainText), this.hashed_password);
...
var authUser = function(database, id, password, callback) {
    ...
    // 2. 패스워드 확인 : 모델 인스턴스를 객체를 만들고 authenticate() 메소드 호출
			...
			var authenticated
				= user.authenticated(password, results[0]._doc.salt, results[0]._doc.hashed_password);
			if(authenticated){
...
var addUser = function(database, id, password, name, callback) {
	console.log('addUser 호출됨 : ' + id + ', ' + password + ', ' + name); //여기까진 정상실행
